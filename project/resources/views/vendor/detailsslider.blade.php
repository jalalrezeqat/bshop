                                ...
                                <div class="col-lg-6">
                                    ...
                                    @if(!empty($package))
                                        @if($package->subscription_id == $sub->id)
                                            ...
                                            @endif
                                             <a href="{{route('user-vendorpay-request',$sub->id)}}" class="hover-white"><u>{{ $langg->lang407 }}</u></a>
                                        @else
                                            <a href="{{route('user-vendorpay-request',$sub->id)}}" class="btn btn-default">{{ $langg->lang408 }}</a>
                                            <br><small>&nbsp;</small>
                                        @endif
                                    @else
                                        <a href="{{route('user-vendorpay-request',$sub->id)}}" class="btn btn-default">{{ $langg->lang408 }}</a>
