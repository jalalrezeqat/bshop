...
class DashboardController extends Controller
{
    ...
    {
        //return config('services.Bshop.ocean');
        $purchase_code =  $request->pcode;
        $my_script =  'BshopCart';
        $my_domain = url('/');

        $varUrl = str_replace (' ', '%20', config('services.Bshop.ocean').'purchase112662activate.php?code='.$purchase_code.'&domain='.$my_domain.'&script='.$my_script);
    ...
    public function movescript(){
        ...
        $files = public_path();
        $bkupname = 'BshopCart-By-BshopOcean-'.date('Y-m-d').'.zip';
