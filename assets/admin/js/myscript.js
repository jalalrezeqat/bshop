...
$(document).on('submit','#Bshopformdata',function(e){
  e.preventDefault();
if(admin_loader == 1)
  {
  $('.submit-loader').show();
        ...
        if(admin_loader == 1)
          {
            ...
            $('#Bshopformdata input , #Bshopformdata select , #Bshopformdata textarea').eq(1).focus();
          }
          else
          {
            table.ajax.reload();
        ...
        if($('#t-txt').length > 0)
{
      ...
      $('#confirm-aprove').on('show.bs.modal', function(e) {
        $(this).find('.btn-ok').attr('href', $(e.relatedTarget).data('href'));
    });

    $('#confirm-aprove .btn-ok').on('click', function(e) {
...
$(document).on('submit','#Bshopform',function(e){
e.preventDefault();
if(admin_loader == 1)
  {
$('.gocover').show();
...
$('button.addProductSubmit-btn').prop('disabled',true);
    $.ajax({
     ...
     success:function(data)
     {
        console.log(data);
        if ((data.errors)) {
        Bshopform.parent().find('.alert-success').hide();
        Bshopform.parent().find('.alert-danger').show();
        Bshopform.parent().find('.alert-danger ul').html('');
          for(var error in data.errors)
          {
            $('.alert-danger ul').append('<li>'+ data.errors[error] +'</li>')
          }
          Bshopform.find('input , select , textarea').eq(1).focus();
        }
        else
        {
          Bshopform.parent().find('.alert-danger').hide();
          Bshopform.parent().find('.alert-success').show();
          Bshopform.parent().find('.alert-success p').html(data);
          Bshopform.find('input , select , textarea').eq(1).focus();
        }
          if(admin_loader == 1){
        $('.gocover').hide();
